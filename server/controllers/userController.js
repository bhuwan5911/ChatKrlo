import { generateToken } from "../lib/utils.js";
import User from "../models/User.js";
import bcrypt from "bcryptjs";
import cloudinary from "../lib/cloudinary.js"
import redisClient from "../lib/redis.js"; 


/*SIGN UP USER*/

// Signup a new user
export const signup = async (req, res) => {
    const { fullName, email, password, bio } = req.body;

    try {
        // Check if all required data is present
        if (!fullName || !email || !password || !bio) {
            return res.json({ success: false, message: "Missing Details" });
        }

        // Check if user already exists
        const user = await User.findOne({ email });
        if (user) {
            return res.json({ success: false, message: "Account already exists" });
        }

        // Hashing user password
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);

        // Creating new user in DB
        const newUser = await User.create({
            fullName,
            email,
            password: hashedPassword,
            bio
        });

        // Generate JWT token
        const token = generateToken(newUser._id);

        // Cache Pre-Warming: Store user in Redis cache
        const cacheKey = `user:${newUser._id}`;
        await redisClient.setEx(cacheKey, 3600, JSON.stringify(newUser));

        res.json({
            success: true,
            userData: newUser,
            token,
            message: "Account created successfully"
        });

    } catch (error) {
        console.log(error.message);
        res.json({ success: false, message: error.message });
    }
};


/* LOGIN USER  */

// Controller to login a user
export const login = async (req, res) =>{
    try {
        const { email, password } = req.body;

        // Check if user exists
        const userData = await User.findOne({ email });
        if (!userData) {
            return res.json({ success: false, message: "Invalid credentials" });
        }

        // Check if password is correct
        const isPasswordCorrect = await bcrypt.compare(password, userData.password);
        if (!isPasswordCorrect) {
            return res.json({ success: false, message: "Invalid credentials" });
        }

        // Generate authentication token
        const token = generateToken(userData._id);

        //Cache Warming: Add user to cache on login
        const cacheKey = `user:${userData._id}`;
        await redisClient.setEx(cacheKey, 3600, JSON.stringify(userData));

        res.json({
            success: true,
            userData,
            token,
            message: "Login successful"
        });

    } catch (error) {
        console.log(error.message);
        res.json({ success: false, message: error.message });
    }
};


/*   CHECK USER AUTH STATUS      */

// Controller to check if user is authenticated (middleware attaches req.user)
export const checkAuth = (req, res) => {
    res.json({ success: true, user: req.user });
};


/*    UPDATE USER PROFILE    */

// Controller to update user profile details
export const updateProfile = async (req, res) => {
    try {
        const { profilePic, bio, fullName } = req.body;
        const userId = req.user._id;

        let updatedUser;

        // CASE 1: No image uploaded → update only text fields
        if (!profilePic) {
            updatedUser = await User.findByIdAndUpdate(
                userId,
                { bio, fullName },
                { new: true }
            );
        } 
        // CASE 2: New image uploaded → upload to Cloudinary + update fields
        else {
            const upload = await cloudinary.uploader.upload(profilePic);
            updatedUser = await User.findByIdAndUpdate(
                userId,
                { profilePic: upload.secure_url, bio, fullName },
                { new: true }
            );
        }

        //Cache Invalidation
        const cacheKey = `user:${userId}`;
        console.log("CACHE INVALIDATION: Deleting", cacheKey);
        await redisClient.del(cacheKey);

        //Redis Pub/Sub: profile update globally
        console.log("PUBSUB: Publishing profile update to 'user-updates'");
        await redisClient.publish("user-updates", JSON.stringify(updatedUser));

        res.json({ success: true, user: updatedUser });

    } catch (error) {
        console.log(error.message);
        res.json({ success: false, message: error.message });
    }
};

/*     GET USER BY ID  */

// Controller to get user details from cache or DB
export const getUserById = async (req, res) => {
    try {
        const { id } = req.params;
        const cacheKey = `user:${id}`;

        // Check Redis cache first
        const cachedUser = await redisClient.get(cacheKey);

        if (cachedUser) {
            console.log("CACHE HIT: Data found in Redis for", id);
            return res.status(200).json(JSON.parse(cachedUser));
        }

        // If not found in cache → fetch from MongoDB
        console.log("CACHE MISS: Fetching from MongoDB for", id);
        const user = await User.findById(id).select("-password");

        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }

        // Store retrieved data in Redis for future requests
        await redisClient.setEx(cacheKey, 3600, JSON.stringify(user));

        // Send response
        res.status(200).json(user);

    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Server error" });
    }
};
